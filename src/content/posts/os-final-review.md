...
1. 一级依据：老师最新 PPT 文字版 `操作系统_期末复习.md`。
2. 二级依据：2022/2023 往年卷、2024 习题课、原复习提纲。
3. 三级依据：William Stallings《操作系统：精髓与设计原理》对应章节概念。

使用说明：

- 这份文件按老师最新 PPT 的考点顺序重建。凡最新 PPT 明确列出的内容，都按必复习处理。
- 往年卷出现但最新 PPT 未列出的“文件系统”等内容，放在最后的低优先级补充区。
- 大题计算的手算模板仍可配合 `操作系统大题模板与真题整理.md` 使用。
...
|---|---:|---:|---|
| 判断题 | 4 | 4 | 盯概念边界和常见陷阱 |
| 选择题 | 10 | 20 | 盯术语、性质、优缺点、算法适用场景 |
| 简答题 | 7 | 42 | 每章准备 2-4 个 6 分模板 |
| 问答和计算题 | 3 | 34 | 信号量、银行家/死锁、页面置换/调度/I/O 调度 |

最重要变化：

- 文件系统没有出现在最新 PPT 主考点中，降级为补充。
- 第五章同步互斥被明确标注“计算题”，信号量、管程、条件变量和典型同步问题要更细。
- 第九/十章增加实时调度算法 EDF/RM。
- I/O 管理增加系统设计目标、缓冲区组织、磁盘访问时间、LIFO/FSCAN。
